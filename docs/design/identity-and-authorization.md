...
| **AU4** ~ | `hdh agent` refuses without login ✅; identity threaded into the Gateway/ChatSession and the care-plan + refill write tools ✅; those tools `require` the permission and attribute to the person, killing the `care-plan review` / `refill (agent)` strings on the agent path ✅; **authorization re-resolved at action time** (each write uses the live signed-in identity) ✅; chart-agent + comprehension-apply tools and `chart history --actor` follow | AU2, AU3 |
