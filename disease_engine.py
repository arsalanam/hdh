...
class RxSpec:
    ...
    dose: str
    frequency: str
    duration_days: Optional[int]   # None = chronic / ongoing
    refills: int = 0
...
def hba1c():
    return [LabSpec("HbA1c", "4548-4", "%", 4.0, 5.6, 5.1, 0.3,
                    condition_shift=2.8, condition_shift_sd=0.8)]

def tsh():
    return [LabSpec("TSH", "3016-3", "mIU/L", 0.4, 4.0, 2.0, 0.8)]

def ua():
    return [LabSpec("UA WBC", "5767-9", "cells/hpf", 0, 5, 1, 1,
                    condition_shift=30, condition_shift_sd=15)]
...
def glucose_stat():
    return [LabSpec("Glucose (stat)", "2345-7", "mg/dL", 70, 100, 95, 10,
                    condition_shift=55, condition_shift_sd=30)]
...
FLU_SEASON = {1:2.5, 2:2.0, 3:1.2, 4:0.5, 5:0.3, 6:0.2,
              7:0.2, 8:0.2, 9:0.5, 10:1.0, 11:1.8, 12:2.5}
RSV_SEASON  = {1:2.0, 2:1.5, 3:0.8, 4:0.4, 5:0.2, 6:0.1,
               7:0.1, 8:0.1, 9:0.5, 10:1.2, 11:2.0, 12:2.5}
...
CONDITIONS: dict[str, ConditionProfile] = {
    ...
    "conjunctivitis": ConditionProfile(
        ...
        pain         = (2, 1),
        labs         = [],
        rx_options   = [RxSpec("Erythromycin ophthalmic ointment","Antibiotic eye ointment","apply","TID",7,0),
    ...
    "rash_eczema": ConditionProfile(
        ...
        pain         = (2, 1),
        labs         = [],
        rx_options   = [RxSpec("Hydrocortisone 1% cream","Topical steroid","apply","BID",14,2),
...
def pick_condition(age: int, month: int, existing_conditions: set[str]) -> ConditionProfile:
    """
