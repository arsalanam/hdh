...
def test_load_counts_and_ledger(loaded_session):
    ...
    assert (total, billable, chapters) == (123, 87, 4)

    ledger = loaded_session.execute(select(_t("ontology_loads"))).mappings().all()
    assert len(ledger) == 1
    assert ledger[0]["concept_count"] == 123
...
def test_hierarchy_path_and_descendants(loaded_session):
    ...
    assert row["path"] == "ch19.S52.S520.S5200.S52001.S52001A"
    assert row["hierarchy_depth"] == 5
    ...
    assert parent_edges == 119  # every non-chapter concept has exactly one
...
def test_reload_requires_force(loaded_session):
    ...
    assert dict(report)["load"].startswith("123")
    ledger = loaded_session.execute(select(_t("ontology_loads"))).mappings().all()
    assert len(ledger) == 2
