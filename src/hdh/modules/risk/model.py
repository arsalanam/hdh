...
from .features import DEFAULT_HORIZON_DAYS, FEATURE_NAMES, extract_features
...
def train(
    ...
    thresholds = {tier: float(np.quantile(train_probs, q)) for tier, q in TIER_QUANTILES.items()}
...
def score(
    session: Session, model_path: str = DEFAULT_MODEL_PATH, mrn: str | None = None, top: int | None = None
) -> list[dict]:
    ...
    if not Path(model_path).exists():
        raise FileNotFoundError(f"No trained model at {model_path} — run `hdh risk train` first.")
    artifact = joblib.load(model_path)

    cutoff = _latest_visit_date(session)
    mrns, rows, _ = extract_features(session, cutoff, artifact["horizon_days"], with_labels=False)
    X = np.asarray(rows, dtype=float)
    probs = artifact["model"].predict_proba(X)[:, 1]

    idx = {name: i for i, name in enumerate(artifact["feature_names"])}
    results = []
    for m, row, prob in zip(mrns, rows, probs, strict=False):
        if mrn and m != mrn:
            continue
        results.append(
            {
