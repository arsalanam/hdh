...
def register_cli(subparsers):
    ...
    tr.add_argument("--horizon", type=int, default=180, help="Prediction horizon in days (default 180)")
...
def run(session, args):
    ...
    try:
        ...
        raise SystemExit(
            f"Risk module dependencies missing ({e.name}). Install with: pip install hdh[risk]"
        ) from None

    if args.risk_cmd == "train":
        print("🧮 Extracting features and training risk model...")
        art = model.train(session, horizon_days=args.horizon, model_path=args.model_path)
        print(f"✅ Model saved → {args.model_path}")
        print(
            f"   Patients: {art['n_patients']:,}  |  "
            f"Positive rate: {art['positive_rate']:.1%}  |  "
            f"Held-out ROC AUC: {art['auc']:.3f}"
        )
        print(
            f"   Tier thresholds: high ≥ {art['thresholds']['high']:.3f}, "
            f"moderate ≥ {art['thresholds']['moderate']:.3f}"
        )

    elif args.risk_cmd == "score":
        rows = model.score(session, model_path=args.model_path, mrn=args.mrn, top=args.top)
        if args.json:
        ...
        print("=" * 88)
        print(
            f"{'MRN':<14}{'Prob':>7}  {'Tier':<10}{'Age':>4}"
            f"{'Chronic':>9}{'Uncontrolled':>14}{'Urgent12m':>11}{'CritLabs':>10}"
        )
        print("-" * 88)
        for r in rows:
            print(
                f"{r['mrn']:<14}{r['risk_probability']:>7.3f}  {r['risk_tier']:<10}"
