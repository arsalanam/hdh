        ...
        if lp is None or (as_of - lp).days > interval:
            ...
            gaps.append(
                CareGap(
                    ...
                    patient_name=name,
                    age=age,
                    gap_type="overdue_preventive",
                    severity="low",
                    description=(
                        f"No preventive visit in {(as_of - lp).days} days (interval: {interval}d)"
                        if lp
            ...
            if days_since > 90:
                ...
                gaps.append(
                    CareGap(
                        ...
                        patient_name=name,
                        age=age,
                        gap_type="uncontrolled_chronic",
                        severity="high",
                        description=(f"Uncontrolled: {conds} — no visit in {days_since} days"),
            ...
            if as_of > deadline:
                gaps.append(
                    CareGap(
                        ...
                        patient_name=name,
                        age=age,
                        gap_type="missed_follow_up",
                        severity="medium",
                        description=(f"Follow-up in {fu_days}d requested on {visit_date} — not seen since"),
            ...
            if days_since > POLYPHARMACY_REVIEW_WINDOW:
                gaps.append(
                    CareGap(
                        ...
                        patient_name=name,
                        age=age,
                        gap_type="polypharmacy_review",
                        severity="medium",
                        description=(
                            f"{n_drugs} distinct medications in the last year, no visit in {days_since} days"
                        ),
